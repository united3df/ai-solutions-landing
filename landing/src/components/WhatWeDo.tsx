    ...
    <section ref={sectionRef} className="py-20 bg-white">
      <div className="container mx-auto px-4 md:px-6">
        <div className={`max-w-4xl mx-auto text-center space-y-6 transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
          ...
          <div className={`pt-6 space-y-3 transition-all duration-700 delay-600 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
            ...
            <ul className="space-y-3 text-slate-600 max-w-xl mx-auto">
              {["Clear scope and predictable behavior", "Maintainable architecture", "Measurable operational improvement"].map((item, i) => (
                <li 
                  key={i}
                  className={`flex items-center gap-3 justify-center transition-all duration-700 ${isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-10'}`}
