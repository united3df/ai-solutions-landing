...
export async function getPostBySlug(slug: string): Promise<PostFull> {
  const res = await fetch(`${getBaseUrl()}/blog/posts/${encodeURIComponent(slug)}`);
  if (!res.ok) {
...
export function adminApi(token: string) {
  ...
  return {
    ...
    async getPost(id: number): Promise<PostFull | null> {
      const res = await fetch(`${base}/admin/posts/${id}`, { headers: headers() });
    ...
    async updatePost(
      id: number,
      body: Partial<{ title: string; content: string; excerpt: string; status: string; meta_title: string; meta_desc: string; metaTitle: string; metaDesc: string }> = {},
    ): Promise<void> {
    ...
    async addTopic(body: { title: string; keyword?: string; priority?: number }): Promise<{ success: boolean; id: number }> {
